...
obs = np.array(game_map.grid.copy())
print(obs.shape)
print(obs[1][1][0].name)

for i in range(16):
    for j in range(16):
        print(obs[i][j][0].name)
...
while running:
    ...
    #move player 3 and 4
    if game_type == "solo_play":
        game_map.player.floodfill(game_map, game_type)
    elif game_type == "team_vs":
        game_map.player3.randomMovement(game_map, game_type)
        game_map.player4.randomMovement(game_map, game_type)
